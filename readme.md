...
`npm run build` produces lazy `dist`, loader, and individually importable `dist-custom-elements` outputs. `npm run verify:package` packs the library, checks contents, installs the tarball in a temporary consumer, and exercises public imports without publishing.
