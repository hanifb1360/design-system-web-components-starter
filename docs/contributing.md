...
Use Node 22.12+ and `npm ci`. Run `npm run dev` or `npm run storybook`; before a pull request run `npm run lint`, `npm run format:check`, `npm run typecheck`, `npm run docs:check`, `npm run performance:check`, `npm test`, `npm run test:browser`, `npm run test:storybook`, `npm run test:visual`, `npm run build:storybook`, `npm run build:examples`, `npm run verify:package`, and `npm run verify:consumers`.
